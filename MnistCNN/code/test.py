...
def run(p_seed=0, p_kernel_size=5, p_logdir="temp"):
    ...
    split = int(0.7 * num_samples)  # 30% for inference
    inference_indices = indices[split:]

    inference_sampler = torch.utils.data.Subset(test_dataset, inference_indices)

    test_loader = torch.utils.data.DataLoader(inference_sampler, batch_size=100, shuffle=False)
